use super::{Opts,EncryptOpts, DecryptOpts};
...
struct Data {
    tmp_dir: TempDir,
    key1: PathBuf, key2: PathBuf, key3: PathBuf, _2outof3: PathBuf,
    output: PathBuf,
}
...
fn setup() -> io::Result<Data> {
    ...
    Ok(Data { tmp_dir, key1, key2, key3, _2outof3, output })        
}

#[test]
fn test_2outof3_roundtrip() -> io::Result<()> {
    let Data{ tmp_dir, key1, key2, key3, output, .. } = setup()?;
...
fn test_2outof3_decrypt_sample() -> io::Result<()> {
    let Data{ tmp_dir, key1, key2, _2outof3, output, .. } = setup()?;
    super::run(&Opts::Decrypt(DecryptOpts {
        input: Some(&_2outof3),
        identities: vec![&key1, &key2],
        output: Some(&output),
